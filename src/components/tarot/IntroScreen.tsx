...
export default function IntroScreen({ onStart }: IntroScreenProps) {
  ...
  return (
    <div className="relative min-h-screen md:h-full flex flex-col items-center justify-center p-4 overflow-hidden">
      ...
      {/* 主要内容 */}
      <div
        className={`relative z-10 text-center transition-all duration-1000 ${isVisible ? "opacity-100" : "opacity-0"}`}
      >
        {/* 图标装饰 */}
        <div
          className={`mb-6 md:mb-8 transition-all duration-700 delay-300 ${showContent ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-8"}`}
        >
        ...
        {/* 标题 */}
        <div
          className={`transition-all duration-700 delay-500 ${showContent ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          ...
          <p className="text-xs md:text-sm text-purple-400/50 max-w-md mx-auto mb-6 md:mb-8 px-2">
        ...
        {/* 功能介绍 */}
        <div
          className={`grid grid-cols-3 gap-2 md:gap-4 max-w-lg mx-auto mb-6 md:mb-10 transition-all duration-700 delay-700 ${showContent ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          {[
            ...
            return (
              <div
                key={i}
                className="flex flex-col items-center gap-1 md:gap-2 p-2 md:p-3 rounded-lg bg-slate-800/30 border border-purple-400/10"
              >
        ...
        {/* 开始按钮 */}
        <div
          className={`transition-all duration-700 delay-900 ${showContent ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          <button
            onClick={onStart}
        ...
        {/* 版权信息 */}
        <div
          className={`mt-8 md:mt-12 text-xs text-purple-400/30 transition-all duration-700 delay-1000 ${showContent ? "opacity-100" : "opacity-0"}`}
        >
      ...
      <style jsx>{`
        .orbit-icon {
          transform: translate(-50%, -50%) rotate(var(--angle))
            translateY(-44px) rotate(calc(-1 * var(--angle)));
        }
        @media (min-width: 768px) {
          .orbit-icon {
            transform: translate(-50%, -50%) rotate(var(--angle))
              translateY(-62px) rotate(calc(-1 * var(--angle)));
          }
        }
        @keyframes twinkle {
          0%,
